...
$(document).ready(function () {
    ...
    $('#btnComplete').click(function () {
        ...
        cart.forEach(item => {
            var sale = { id: 0, saleid: item.id, name: item.name, price: item.price, quantity: item.quantity, fullname: name, email: emailAdd };
...
function updateQuantity(id, obj) {
    if (Number(obj.value) === 0) {
        ...
        cart.forEach(v => {
            if (v.id === id) v.quantity = Number(obj.value);
            _displayCount();
            _displayCart();
        });
    }
